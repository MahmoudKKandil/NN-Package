    ...
    while(count > -1):
        ...
        for i in range(nomOfNurons):
            if(count == NumberOfLayers):
                sNet = (TrainY[i]-arrayOfnetarrays[count][i]) * \
                    BackActivationFunction(
                        arrayOfnetarrays[count][i], ActivFunction)
            else:
                for j in range(len(oldS)):
                    sNet += (oldS[j]*weights[count+1][j][i+1])
                sNet *= BackActivationFunction(
                    arrayOfnetarrays[count][i], ActivFunction)
            newS.append(sNet)
            sNet = 0
        count -= 1
...
def Train(trainX, TrainY, l_rate, n_epoch, NumberOfLayers, neurons, numberOfFeatures, bias, ActivFunction):
    ...
    for epoch in range(n_epoch):
        for i, row in enumerate(trainX):
            arrayOfnetarrays = forward(
                NumberOfLayers, row, weights, bias, ActivFunction)
            arrOfSArrays = back(arrayOfnetarrays, ActivFunction,
                                weights, TrainY[i], NumberOfLayers)
            arrOfSArrays = arrOfSArrays[::-1]
            # ana 3mlt reverse llarray 3shan 25li ellocal gradient bta3 eloutput hwa eli fl25er w yb2a bnfs tarteb elarray eli feh elnet
            res = updatewights(arrOfSArrays, arrayOfnetarrays,
                               weights, l_rate, bias, NumberOfLayers, row)
...
def test_Data(testX, testY, weights, NumberOfLayers, bias, ActivFunction):
    ...
    for i, row in enumerate(testX):
        arrayOfnetarrays = forward(
            NumberOfLayers, row, weights, bias, ActivFunction)
        result = np.where(arrayOfnetarrays[NumberOfLayers] == np.amax(
            arrayOfnetarrays[NumberOfLayers]))
